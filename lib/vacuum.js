...
class vacuum {
	...
	async command(duid, parameter, value)
	{
		if (parameter == "load_multi_map") {
			...
			this.adapter.log.debug("Room cleaning floor: " + roomFloor);


			await rr_mqtt_connector.sendRequest(duid, rr, "get_room_mapping", []).then(async attribute_val => {
				this.adapter.log.debug("Room cleaning room_mapping: " + JSON.stringify(attribute_val));
				for (const mappedRoom in attribute_val)
				{
			...
			this.adapter.log.debug("Room cleaning roomList: " + JSON.stringify(roomList));
