		...
		try {
			if (targetFolder == "networkInfo") {
				...
				for (const attribute in deviceStatus[0]) {
					...
					switch (attribute) {
						case "dock_type":
							this.adapter.log.debug(`TEST TEST TEST processing dock_type of get_status ${JSON.stringify(deviceStatus)}`);
							this.adapter.log.debug(`TEST TEST TEST attribute ${JSON.stringify(attribute)}`);
