...
class vacuum {
		...
		switch (parameter) {
			case "load_multi_map":
				await this.rr_mqtt_connector.sendRequest(duid, "load_multi_map", value).then(async (result) => {
					...
					this.adapter.log.warn("Failed to execute load_multi_map. Error: " + error);
				});
				break;
					...
					await this.rr_mqtt_connector.sendRequest(duid, "get_room_mapping", []).then(async mappedRoomList => {
						...
						await this.adapter.getStateAsync("Devices." + duid + ".floors.cleanCount").then(async cleanCount => {
							...
							await this.rr_mqtt_connector.sendRequest(duid, parameter, [roomList]).then(result => {
								...
								this.adapter.log.warn("Failed to execute app_segment_clean. Error: " + error);
							});
						});
					}).catch(error => {
						this.adapter.log.warn("Failed to execute get_room_mapping. Error: " + error);
					});
				});
				break;
			...
			case "reset_consumable":
				try{
					...
					await this.rr_mqtt_connector.sendRequest(duid, parameter, [value]).then(async parameter => {
						...
						this.adapter.log.warn("Failed to execute reset_consumable. Error: " + error);
					});
				}
				catch (e) {
			...
			case "app_set_dryer_status":
				await this.rr_mqtt_connector.sendRequest(duid, parameter, ).then(result => {
					...
					this.adapter.log.errwarnor("Failed to execute " + parameter + ". Error: " + error);
				});
				break;
			default:
				if ((value) && (typeof(value) != "boolean")) {
					...
					await this.rr_mqtt_connector.sendRequest(duid, parameter, value).then(result => {
						...
						this.adapter.log.warn("Failed to execute " + parameter + ". Error: " + error);
					});
				}
				else {
					await this.rr_mqtt_connector.sendRequest(duid, parameter, ).then(result => {
	...
	async getParameter(duid, parameter, attribute) {
		...
		if (targetFolder == "networkInfo")
		{
			...
			await this.rr_mqtt_connector.sendRequest(duid, "get_consumable", []).then(async consumables => {
				...
				this.adapter.log.warn("Failed to execute get_consumable. Error: " + error);
			});
		}
			...
			await this.rr_mqtt_connector.sendRequest(duid, "get_status", []).then(deviceStatus => {
				...
				this.adapter.log.warn("Failed to execute get_status. Error: " + error);
			});
		}
			...
			await this.rr_mqtt_connector.sendRequest(duid, "get_multi_maps_list", []).then(mapList => {
				...
				this.adapter.log.warn("Failed to execute get_multi_map_list. Error: " + error);
			});
		}
			...
			await this.rr_mqtt_connector.sendRequest(duid, parameter, []).then(async firmwareFeatures => {
				...
				this.adapter.log.warn("Failed to execute get_fw_features. Error: " + error);
			});
		}
			...
			this.rr_mqtt_connector.sendRequest(duid, "get_photo" , {
				...
				this.adapter.log.debug("Photo response !!!!!!!!!!!!! " + JSON.stringify(photoresponse));
			}).catch(e => {
	...
	async getCleanSummary(duid) {
		await this.rr_mqtt_connector.sendRequest(duid, "get_clean_summary", []).then(async cleaningAttributes => {
					...
					case "3":
						for (const cleaningRecord in cleaningAttributes[cleaningAttribute])
						{
							await this.rr_mqtt_connector.sendRequest(duid, "get_clean_record", [cleaningAttributes[cleaningAttribute][cleaningRecord]]).then(async cleaningRecordAttributes => {
								...
								this.adapter.log.warn("Failed to execute get_clean_record. Error: " + error);
							});
						}
			...
			this.adapter.log.warn("Failed to execute get_clean_summary. Error: " + error);
		});
	}
