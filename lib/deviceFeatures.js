...
class deviceFeatures {
	...
	isDustCollectionSettingSupported() {
		this.adapter.log.debug(`TEST TEST TEST isDustCollectionSettingSupported`);
	...
	getFeatureList() {
		return {
			...
			isCornerCleanModeSupported: !!(2147483648 & parseInt("0x" + this.featuresStr.slice(-8))),
			isDssBelievable: !!(131072 & parseInt("0x" + this.featuresStr.slice(-8))),
			...
			isBackChargeAutoWashSupported: !!(4096 & parseInt("0x" + this.featuresStr.slice(-8))),
			isAvoidCarpetSupported: [
				"roborock.vacuum.a40", // Q7
				"roborock.vacuum_s6", // S6
				"roborock.vacuum.a72", // Q5 Pro
	...
	processDockType(dockType) {
		...
		this.adapter.log.debug(`TEST TEST TEST processDockType before model type`);
		if (this.model == "roborock.vacuum.a15") {
			this.adapter.log.debug(`TEST TEST TEST processDockType after model type`);
			this.adapter.log.debug(`TEST TEST TEST dockType ${dockType}`);
			switch (dockType) {
				case 0: // Charging dock
					// nothing to do here
					break;
				case 1: // Auto-Empty Dock
					this.adapter.log.debug(`TEST TEST TEST case 1`);
